...
def plot_bootstrap_results(out_dir, label, fig, axes):
    ...
    if PLOT_ABSOLUTE_ERROR:
        ...
        LEVELS = np.array([0.02])  # Proportion absolute error
        ...
        for ax, label, data in zip(axes.ravel(), LABELS, datasets):
            ax.set_title(label)
            # ax.set_xlabel('Proportion Reference Population 1 ($p_1^*$)')
            # ax.set_xlabel('$p_1^*$')
...
#     ax_ci = fig_vio.add_subplot(111)
    ...
    fig_select = plt.figure(figsize=(12, 12))
    gs = plt.GridSpec(nrows=len(sizes), ncols=2, width_ratios=[2, 1], hspace=0.2, wspace=0.005)
...
#            g = sns.violinplot(x='Estimate', y='Size', hue='Method', data=df, ax=ax_vio, orient='h', cut=0, linewidth=2)
            g = sns.violinplot(x='Estimate', y='Method', data=df, ax=ax_vio, orient='h', cut=0, linewidth=2)
            g.axvline(x=p_star, ymin=0, ymax=1, ls='--')  # ,
                       #label="Ground Truth: {:3.2}".format(p_star))
#            handles, labels = g.get_legend_handles_labels()
#            g.legend(handles, labels[:len(methods)], title="Method")
